...
let input: string = String(Color.WHITE);
...
function main() {
  ...
  for (let y = maxY + 2; y >= minY - 2; y--) {
    for (let x = minX - 1; x <= maxX + 1; x++) {
      const piece = grid.get(key({ x: y, y: x }));
      if (!piece) {
        process.stdout.write("██");
      } else {
        process.stdout.write(piece.color === Color.WHITE ? "░░" : "██");
      }
    }
    process.stdout.write("\n");
  }
...
function getGridSizes(grid: Grid) {
  return [...grid.keys()].reduce(
    (acc, cur) => {
      const [x, y] = cur.split(",").map(Number);
      if (x > acc.maxX) return { ...acc, maxX: x };
      if (x < acc.minX) return { ...acc, minX: x };
      if (y > acc.maxY) return { ...acc, maxY: x };
      if (y < acc.minY) return { ...acc, minY: x };
      return acc;
    },
    {
